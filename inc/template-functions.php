<?php
/**
 * Functions and definitions for ForBlogs theme.
 *
 * @author Terry Lin
...
if ( ! function_exists( 'ForBlogs_remove_invalid_rel_for_category' ) ) {
	/**
	 ...
	 * @return string|array
	 */
	function ForBlogs_remove_invalid_rel_for_category( $var ) {
		return str_replace( 'rel="category tag"', 'rel="tag"', $var );
	}
}

if ( ! function_exists( 'ForBlogs_add_slug_to_body_class' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ForBlogs_add_slug_to_body_class( $classes ) {
		global $post;
...
if ( ! function_exists( 'ForBlogs_remove_recent_comments_style' ) ) {
	/**
	 * Remove wp_head() injected Recent Comment styles.
	 *
	 * @return void
	 */
	function ForBlogs_remove_recent_comments_style() {
...
if ( ! function_exists( 'ForBlogs_remove_thumbnail_dimensions' ) ) {
	/**
	 ...
	 * @return string
	 */
	function ForBlogs_read_more( $more ) {
		global $post;
		return '... &raquo; <a class="read-more-link" href="' . esc_url( get_permalink( $post->ID ) ) . '">' . __( 'read more', 'ForBlogs' ) . '</a>';
	}
}
...
if ( ! function_exists( 'ForBlogs_remove_thumbnail_dimensions' ) ) {
	 ...
	 * @return string
	 */
	function ForBlogs_remove_thumbnail_dimensions( $html ) {
		$html = preg_replace( '/(width|height)=\"\d*\"\s/', '', $html );
		return $html;
	}
}

if ( ! function_exists( 'ForBlogs_custom_gravatar' ) ) {
	/**
	 * Custom avatar
	 *
	 ...
	 * @return array
	 */
	function ForBlogs_custom_gravatar( $avatar_defaults ) {
...
if ( ! function_exists( 'ForBlogs_enable_threaded_comments' ) ) {
	/**
	 * Threaded Comments
	 *
	 * @return void
	 */
	function ForBlogs_enable_threaded_comments() {
		if ( ! is_admin() ) {
...
if ( ! function_exists( 'ForBlogs_comment' ) ) {
	/**
	 ...
	 * @return void
	 */
	function ForBlogs_comment( $comment, $args, $depth ) {
		global $post;
...
if ( ! function_exists( 'ForBlogs_comment_form' ) ) {
	/**
	 * Overwrite the default WordPress comment form.
	 * Build a custom ForBlogs style form.
	 *
	 * @return array
	 */
	function ForBlogs_comment_form() {
		$comment_field = '
			<div class="form-row">
		...
		$comments_args = array(
			'label_submit'         => __( 'Send', 'ForBlogs' ),
			'title_reply'          => __( 'Write a Reply or Comment', 'ForBlogs' ),
			'comment_notes_after'  => '',
			'comment_field'        => $comment_field,
			'title_reply_before'   => '<h3 id="reply-title" class="section-title">',
			'title_reply_after'    => '</h3>',
			'class_submit'         => 'btn btn-green my-1',
			'comment_notes_before' => '<p class="comment-notes">' . __( 'Your email address will not be published.', 'ForBlogs' ) . '</p>',
		);
		return $comments_args;
	}
}

if ( ! function_exists( 'ForBlogs_comment_fileds' ) ) {
	/**
	 * Comment fields
	 * 
	 * @return array
	 */
	function ForBlogs_comment_fileds() {
		...
		$author = '
			<div class="col-sm-6 my-1">
				<div class="input-group">
					...
					<input id="author" class="form-control" placeholder="' . esc_attr( __( 'Name', 'ForBlogs' ) ) . '" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" maxlength="245" />
				</div>
			</div>
		';
		...
		$email = '
			<div class="col-sm-6 my-1">
				<div class="input-group">
					...
					<input id="email" class="form-control" placeholder="' . esc_attr( __( 'Email', 'ForBlogs' ) ) . '" name="email" type="text" value="' . esc_attr( $commenter['comment_author_email'] ) . '" maxlength="100" aria-describedby="email-notes" />
				</div>
			</div>
		';
		...
		$url = '
			<div class="col-sm-12 my-1">
				<div class="input-group">
					...
					<input id="url" class="form-control" placeholder="' . esc_attr( __( 'Website', 'ForBlogs' ) ) . '" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" maxlength="200" />
				</div>
			</div>
		';
...
if ( ! function_exists( 'ForBlogs_move_comment_field_to_bottom' ) ) {
	/**
...
if ( ! function_exists( 'ForBlogs_replace_language_attributes' ) ) {
	/**
	 * Replace language attrbute from en_US to en.
	 *
	 * @return string
	 */
	function ForBlogs_replace_language_attributes() {
		return 'lang=' . substr( get_bloginfo( 'language' ), 0, 2 );
	}
}

if ( ! function_exists( 'ForBlogs_single_post_script' ) ) {
	/**
	 ...
	 * @return void
	 */
	function ForBlogs_single_post_script() {
		if ( is_single() ) {
	?>
		<script>
...
if ( ! function_exists( 'ForBlogs_scrolling_script' ) ) {
	/**
	 ...
	 * @return void
	 */
	function ForBlogs_scrolling_script() {
		?>
		<script>
		...
		jQuery( document ).ready(function( $ ) {
			...
			$( '#ForBlogs-nav-bar' ).on( 'show.bs.collapse' , function () {
				$( 'body' ).addClass( 'menu-is-collapsed' );
			});

			$( '#ForBlogs-nav-bar' ).on( 'hidden.bs.collapse' , function () {
...
if ( ! function_exists( 'ForBlogs_alx_embed_html' ) ) {
	/**
	 ...
	 * @return string
	 */
	function ForBlogs_alx_embed_html( $html ) {
 ...
 * @return string
 */
function ForBlogs_sanitize_comment( $comment_text ) {
