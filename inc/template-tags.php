<?php
/**
 * Custom template tags for ForBlogs theme.
 *
 * @author Terry Lin
 ...
 */
  ...
  * - ForBlogs_author_posted_date
  * - ForBlogs_author_card
  * - ForBlogs_site_info
  */
 ...
 */
function ForBlogs_nav( $position = 'header' ) {
	if ( 'header' === $position ) {

		if ( class_exists( 'ForBlogs_Walker' ) ) {
			wp_nav_menu(
				array(
					'theme_location'  => 'header-menu',
					'container'       => 'div',
					'container_class' => 'collapse navbar-collapse',
					'container_id'    => 'ForBlogs-nav-bar',
					'menu_class'      => 'navbar-nav mr-auto',
					'menu_id'         => '',
					'depth'           => 2,
					'fallback_cb'     => 'ForBlogs_Walker::fallback',
					'walker'          => new ForBlogs_Walker(),
				)
			);
		} else {
			wp_nav_menu(
		...
		if ( has_nav_menu( 'footer-menu' ) ) {
			wp_nav_menu(
				array(
					...
					'container_id'    => 'ForBlogs-footer-nav',
 ...
 */
function ForBlogs_default_nav() {
	?>
	<div id="ForBlogs-nav-bar" class="collapse navbar-collapse">
		<ul id="menu-primary-menu" class="navbar-nav mr-auto">
			<li class="nav-item"><a href="<?php echo esc_url( home_url() ); ?>" class="nav-link"><?php esc_html_e( 'Home', 'ForBlogs' ); ?></a></li>
		</ul>
	</div>
	<?php
}
...
/**
 * The ForBlogs Post thumbnail.
 */
function ForBlogs_post_thumbnail() {
	$size = 'ForBlogs-thumbnail';

	if ( is_home() || is_front_page() ) {
		if ( '2' === get_theme_mod( 'layout_cols_per_row_home') ) {
			$size = 'ForBlogs-medium';
		}
...
function ForBlogs_pagination( $range = 1 ) {
	...
	if ( 1 !== $pages ) {
		...
		<nav aria-label="Page navigation" role="navigation">
			<span class="sr-only"><?php esc_html_e( 'Page navigation', 'ForBlogs' ); ?></span>
			<ul class="pagination justify-content-center ft-wpbs">
				...
				<li class="page-item">
					<a class="page-link" href="<?php echo esc_url( get_pagenum_link( 1 ) ); ?>" aria-label="<?php esc_attr_e( 'First Page', 'ForBlogs' ); ?>">
						&laquo;<span class="hidden-sm-down d-none d-md-inline-block">&nbsp;<?php esc_html_e( 'First', 'ForBlogs' ); ?></span>
					</a>
				</li>
				<?php endif; ?>
				...
				<?php if ( $current > 1 && $pagi_items < $pages ) : ?>
					<li class="page-item">
						<a class="page-link" href="<?php echo esc_url( get_pagenum_link( $current - 1 ) ); ?>" aria-label="<?php esc_attr_e( 'Previous Page', 'ForBlogs' ); ?>">
							&lsaquo;<span class="hidden-sm-down d-none d-md-inline-block">&nbsp;<?php esc_html_e( 'Previous', 'ForBlogs' ); ?></span>
						</a>
					</li>
				<?php endif; ?>
						...
						<?php if ( $current === $i ) : ?>
							<li class="page-item active">
								<span class="page-link">
									<span class="sr-only"><?php esc_html_e( 'Current Page', 'ForBlogs' ); ?></span>
									<?php echo esc_html( $i ); ?>
								</span>
							</li>
						<?php else : ?>
							<li class="page-item">
								<a class="page-link" href="<?php echo esc_url( get_pagenum_link( $i ) ); ?>">
									<span class="sr-only"><?php esc_html_e( 'Page', 'ForBlogs' ); ?></span>
									<?php echo esc_html( $i ); ?>
								</a>
							</li>
						<?php endif; ?>
				...
				<?php if ( $current < $pages && $pagi_items < $pages ) : ?>
					<li class="page-item">
						<a class="page-link" href="<?php echo esc_url( get_pagenum_link( $current + 1 ) ); ?>" aria-label="<?php esc_attr_e( 'Next Page', 'ForBlogs' ); ?>">
							<span class="hidden-sm-down d-none d-md-inline-block"><?php esc_html_e( 'Next', 'ForBlogs' ); ?>&nbsp;</span>&rsaquo;
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( $current < $pages - 1 && $current + $range - 1 < $pages && $pagi_items < $pages ) : ?>
					<li class="page-item">
						<a class="page-link" href="<?php echo esc_url( get_pagenum_link( $pages ) ); ?>" aria-label="<?php esc_attr_e( 'Last Page', 'ForBlogs' ); ?>">
							<span class="hidden-sm-down d-none d-md-inline-block"><?php esc_html_e( 'Last', 'ForBlogs' ); ?>&nbsp;</span>&raquo;
						</a>
					</li>
				<?php endif; ?>
			</ul>
 ...
 * @return bool
 */
function ForBlogs_is_text_fade_out() {
 ...
 * @return void
 */
function ForBlogs_title_progress_bar() {
	?>
 ...
 * @return void
 */
function ForBlogs_article_schema( $schema = '' ) {
 ...
 * @return void
 */
if ( ! function_exists( 'ForBlogs_edit_button' ) ) {
	function ForBlogs_edit_button() {
		global $post;
		...
		echo '
			<a href="' . esc_url( get_edit_post_link() ) . '" class="button-like-link">
				<div class="btn-counter text-only">		
					<div class="btn">' . esc_html__( 'Edit', 'ForBlogs' ) . '</div>
				</div>
			</a>
		';
	}
 ...
 * @return void
 */
function ForBlogs_column_control_button() {
	?>
 ...
 * @return void
 */
if ( ! function_exists( 'ForBlogs_comment_button' ) ) {
	function ForBlogs_comment_button( $show_label = false ) {
		...
		echo '
			<a href="' . esc_url( get_the_permalink() ) . '#comments" class="button-like-link">
				<div class="btn-counter">
					<div class="btn">
						' . ( ( $show_label ) ? '<i class="fas fa-comment-dots"></i> ' . esc_html__( 'Comment', 'ForBlogs' ) : '<i class="fas fa-comment-dots"></i>' ) . '
					</div>
 ...
 * @return void
 */
if ( ! function_exists( 'ForBlogs_read_button' ) ) {
	function ForBlogs_read_button() {
		echo '
			<a href="' . esc_url( get_the_permalink() ) . '" class="button-like-link">
				<div class="btn-counter text-only">		
					<div class="btn">' . esc_html__( 'Read', 'ForBlogs' ) . '</div>
				</div>
			</a>
		';
	}
 ...
 * @return void
 */
function ForBlogs_posted_date_button( $show_label = false ) {
	echo '
		<div class="btn-counter">
			<div class="btn">
				<i class="far fa-calendar-alt"></i> ' . ( ( $show_label ) ? esc_html__( 'Date', 'ForBlogs' ) : '' ) . '
			</div>
 ...
 * @return void
 */
if ( ! function_exists( 'ForBlogs_author_posted_date' ) ) {
	function ForBlogs_author_posted_date( $show_avatar = false, $avatar_size = 40 ) {
		...
		printf( '<a href="%1$s" title="written %2$s" class="author-link">%3$s</a> <time itemprop="datePublished" datetime="%4$s">%5$s</time>',
			...
			sprintf( esc_html__( '%1$s @ %2$s', 'ForBlogs' ),
 ...
 * Site logo.
 *
 * ForBlogs dosn't use the_custom_logo(), the reason is because it supports 4.5 or up.
 * I hope even lower version of WordPress is able to use this theme as well.
 *
 * @return string Logo Url
 */
function ForBlogs_site_logo() {
...
if ( ! function_exists( 'ForBlogs_author_card' ) ) {
	/**
	 * The author card.
	 *
	 ...
	 * @return void
	 */
	function ForBlogs_author_card( $avatar_size = 96, $icon_size = 'sm' ) {
		$description = get_the_author_meta( 'description' );
		?>
			<h3 class="section-title"><?php esc_html_e( 'Author', 'ForBlogs' ); ?></h3>
 ...
 * @return void
 */
function ForBlogs_category_labels() {
 ...
 * @return void
 */
if ( ! function_exists( 'ForBlogs_site_info' ) ) {
	function ForBlogs_site_info() {
		echo esc_html__( 'Copyright', 'ForBlogs' ) . ' &copy; ' . date( 'Y' ) . ' <strong><a href="' . esc_url( get_site_url() ) . '">' . get_bloginfo( 'name' ) . '</a></strong>. ' . esc_html__( 'All rights reserved.', 'ForBlogs' ) . ' ';

		// Keeping the theme credit link encourages me to improve this theme better. Thank you.
		$theme_link = 'https://terryl.in/';
		echo esc_html__( 'Theme by', 'ForBlogs' ) . ' <a href="' . esc_url( $theme_link ) . '" target="_blank">' . esc_html__( 'ForBlogs', 'ForBlogs' ) . '</a>. ';
	}
}
 ...
 * @return void
 */
function ForBlogs_post_breadcrumb() {
	global $post;

	if ( ForBlogs_is_post_breadcrumb() && is_singular() ) {
				...
				<ul class="breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">
					<li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
						<a href="<?php echo esc_url( get_home_url() ); ?>" itemprop="item">
							<span itemprop="name"><i class="fas fa-globe"></i><span class="sr-only"><?php esc_html_e( 'Home', 'ForBlogs' ); ?></span></span>
						</a>
 ...
 * @return string
 */
function ForBlogs_main_container_css() {
 ...
 * @return string
 */
function ForBlogs_layout_columns() {
 ...
 * @return bool
 */
function ForBlogs_is_post_card_footer() {
 ...
 * @return bool
 */
function ForBlogs_is_post_card_header() {
 ...
 * @return bool
 */
function ForBlogs_is_post_breadcrumb() {
 ...
 * @return bool
 */
function ForBlogs_is_post_author_date() {
 ...
 * @return bool
 */
function ForBlogs_is_post_featured_image() {
 ...
 * @return bool
 */
function ForBlogs_is_post_author_card() {
 ...
 * @return bool
 */
function ForBlogs_is_post_comment_section() {
 ...
 * @return bool
 */
function ForBlogs_is_responsive() {
