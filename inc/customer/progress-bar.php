<?php
/**
 *  ForBlogs theme customizer: Progress Bar
 *
 * @author Terry Lin
...
function ForBlogs_customize_progress_bar( $wp_customize ) {
     ...
     * Panel
     */
	$wp_customize->add_panel( 'panel_ForBlogs_progress_bar', array(
		'title'       => __( 'Progress Bar', 'ForBlogs' ),
		'priority'    => 10,
	));
	...
	$wp_customize->add_section( 'section_progress_bar_basic', 
		array(
			'title'       => __( 'Basic Settings', 'ForBlogs' ),
			'description' => __( 'The basic settings of the page progress bar.', 'ForBlogs' ),
			'priority'    => 10,
			'panel'       => 'panel_ForBlogs_progress_bar',
		)
	);

	$wp_customize->add_section( 'section_progress_bar_color', 
		array(
			'title'       => __( 'Color', 'ForBlogs' ),
			'description' => __( 'Customize the color pattern of the page progress bar.', 'ForBlogs' ),
			'priority'    => 10,
			'panel'       => 'panel_ForBlogs_progress_bar',
		)
	);
	...
	$wp_customize->add_control(
		new Customize_Toggle_Control( $wp_customize, 'progressbar_is_display_bar_control',
			array(
				'label'       => __( 'Display Progress Bar', 'ForBlogs' ),
				'section'     => 'section_progress_bar_basic',
				'settings'    => 'progressbar_is_display_bar',
				'description' => __( 'Would you like to display a progress bar while reading?', 'ForBlogs' ),
			)
		)
	);
	...
	$wp_customize->add_control(
		new Customize_Toggle_Control( $wp_customize, 'progressbar_is_display_percentage_control',
			array(
				'label'       => __( 'Display Percentage Number', 'ForBlogs' ),
				'section'     => 'section_progress_bar_basic',
				'settings'    => 'progressbar_is_display_percentage',
				'description' => __( 'Would you like to display a percentage number on the progress bar?', 'ForBlogs' ),
			)
		)
	);
	...
	$wp_customize->add_control(
		new WP_Customize_Control( $wp_customize, 'progressbar_preferred_color_control',
			array(
				'label'       => __( 'Color Pattern', 'ForBlogs' ),
				'section'     => 'section_progress_bar_basic',
				'settings'    => 'progressbar_preferred_color',
				'type'        => 'radio',
				'description' => __( 'Choose a preferred color pattern and apply it to the progress bar.', 'ForBlogs' ),
				'choices'     => array(
					'default' => __( 'Default', 'ForBlogs' ),
					'menu'    => __( 'As same as website menu', 'ForBlogs' ),
					'custom'  => __( 'Custom', 'ForBlogs' ),
				),
			)
		)
	);
	...
	$wp_customize->add_control(
		new Customize_Alpha_Color_Control( $wp_customize, 'progressbar_custom_bg_color_control', 
			array(
				'label'        => __( 'Background Color', 'ForBlogs' ),
	...
	$wp_customize->add_control(
		new Customize_Alpha_Color_Control( $wp_customize, 'progressbar_custom_border_color_control', 
			array(
				'label'        => __( 'Border Color', 'ForBlogs' ),
				'section'      => 'section_progress_bar_color',
